...
import models
from utils.util import create_logger, AverageMeter, clustering_acc, save_checkpoint, load_checkpoint
...
import models
from utils.util import create_logger, AverageMeter, clustering_acc, save_checkpoint, load_checkpoint, save_list, \
	load_list, process_result, show_result
from utils.sampling import get_pair_EX
...
def main():
	...
	print("==============" + " Clustering for Task " + str(args.order) + " ==============")

	model = models.__dict__[args.arch](num_classes=args.num_classes).cuda()

	abs_path = os.path.abspath('.')
	model_args = abs_path + "/checkpoint/trained_parameters.pth.tar"
